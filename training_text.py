...
import numpy as np
from model_text import TextModel, DistilCamembertEmbedding, FlaubertEmbedding
...
if __name__ == "__main__":
    ...
    model = TextModel(args.output_embedding_model_shape, args.num_classes).to(device)
    ...
    if args.save_model:
        ...
        plt.legend()
        plt.savefig("data/ModelText/Graphs/text_model_loss.png")
        torch.save(model.state_dict(), "data/ModelText/Models/text_model_Early.pt")
        ...
        if len(models_parameters) > 0:
            pocket_model = TextModel(args.output_embedding_model_shape, args.num_classes).to(device)
            pocket_model.load_state_dict(models_parameters[-1])
            torch.save(models_parameters[-1], "data/ModelText/Models/text_model_Early_Pocket_Algo.pt")
